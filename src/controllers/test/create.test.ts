...
import * as helper from '@sumaniac28/gigglobal-helper-v1';
...
jest.mock('@sumaniac28/gigglobal-helper-v1');
  ...
  describe('message method', () => {
    it('should throw an error for invalid schema data', async () => {
      ...
      message(req, res).catch(() => {
        expect(helper.BadRequestError).toHaveBeenCalledWith('This is an error message', 'Create message() method error');
      });
    });
    ...
    it('should throw file upload error', async () => {
      ...
      message(req, res).catch(() => {
        expect(helper.BadRequestError).toHaveBeenCalledWith('File upload failed. Try again!', 'Create message() method error');
      });
    });
